...
`//third_party/tls/{host,android}:openssl` 切片，杜绝第二 TLS 栈。

## 公共 API

```cpp
// src/public/include/http/websocket.h
...
```text
cpp_network_deps.bzl (lws pin v4.5.8) ─► third_party/lws/{host,android}:build_lws genrule
    │  CMake: LWS_WITHOUT_SERVER/EXTENSIONS、静态、指向 bundle OpenSSL 根
    ▼
//third_party/lws/{host,android}:lws (cc_library, alwayslink 于 src/websocket)
    ▼
