...
ErrorCode ClassifyConnectFailure(const char* reason) {
  ...
  if (r.find("ssl") != std::string::npos ||
      r.find("handshake") != std::string::npos) {
      ...
      session->Fail(
          Error(ClassifyConnectFailure(static_cast<const char*>(in)),
                in != nullptr
                    ...
                    : std::string("client connect failed")));
      break;
    case LWS_CALLBACK_CLIENT_WRITEABLE: {
      ...
      if (written < 0) {
        session->tx_failure = Error(
            ...
            "lws_write failed (" + std::to_string(written) + ")");
...
const struct lws_protocols kProtocols[] = {
    {kProtocolName, WsCallback, sizeof(void*) /* session userdata */, 8192,
     0, nullptr, 0},
...
Result<UrlParts> ParseWsUrl(const std::string& url) {
  UrlParts parts;
  const char kSecure[] = "wss://";
  const char kPlain[] = "ws://";
  if (url.rfind(kSecure, 0) == 0) {
    ...
    return Result<UrlParts>::Err(Error(ErrorCode::kInvalidArgument,
                                       "WebSocket URLs must be ws:// or "
                                       "wss://"));
  }
  std::string rest = url.substr(url.find("://") + 3);
  if (rest.empty() || rest[0] == '/' || rest[0] == ':') {
    return Result<UrlParts>::Err(Error(ErrorCode::kInvalidArgument,
                                       "missing host in WebSocket URL"));
  }
  ...
  if (parts.host.empty()) {
    return Result<UrlParts>::Err(
        Error(ErrorCode::kInvalidArgument, "missing host"));
  }
...
Result<WebSocket> WebSocket::Connect(const std::string& url,
      ...
      if (Tls::IsPemText(*tls.client_cert())) {
        ...
        return Result<WebSocket>::Err(
            Error(ErrorCode::kInvalidArgument,
                  "failed to read client certificate/key files"));
      }
  ...
  if (session->context == nullptr) {
    return Result<WebSocket>::Err(
        Error(ErrorCode::kInternalError, "lws_create_context failed"));
  }
  ...
  if (session->wsi == nullptr) {
    session->Fail(Error(ErrorCode::kDnsResolutionFailed,
                        "connect dispatch failed for host: " + parts.host));
  }
  ...
  if (session->state == WsState::kConnecting) {
    session->Fail(Error(ErrorCode::kConnectionTimeout,
  ...
  while (!pred()) {
    ...
    if (session->state == WsState::kClosed && !pred()) {
      ...
      if (session->peer_close_code != 0) {
        Error e(ErrorCode::kConnectionClosed, "connection closed by peer");
...
Error ClosedStateError(WsSession* session, const char* what) {
  if (session->peer_close_code != 0) {
    Error e(ErrorCode::kConnectionClosed,
            std::string(what) + ": connection closed by peer");
    e.set_close_code(session->peer_close_code);
    e.set_close_reason(session->peer_close_reason);
    return e;
  }
...
Result<void> WebSocket::Send(const WsMessage& msg) {
  ...
  if (session->state != WsState::kOpen ||
    ...
    return Result<void>::Err(Error(ErrorCode::kWriteTimeout,
...
Result<void> WebSocket::Close(WsCloseCode code, const std::string& reason) {
  ...
  session->close_reason = reason.substr(0, 123);
  session->state = WsState::kClosing;
