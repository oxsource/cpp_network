...
using cpp_network::ws::WebSocket;
using cpp_network::ws::WsCloseCode;
using cpp_network::ws::WsMessage;
...
bool E1_SiteGet(std::string* detail) {
  ...
  if (!res.ok()) {
    *detail = "[" +
              std::string(cpp_network::comm::ErrorCodeToString(
                  res.error().code())) +
              "] " + res.error().message();
    return false;
  }
...
bool W7_MtlsPair(std::string* detail) {
  ...
  no_cert.SetTls(Tls::Builder().SetVerifyMode(
      cpp_network::comm::VerifyMode::kSkipVerification).Build());
  ...
  with_cert.SetTls(Tls::Builder()
                       .SetVerifyMode(
                           cpp_network::comm::VerifyMode::kSkipVerification)
...
bool S4_MtlsRejectedWithoutClientCert(std::string* detail) {
  Options options = ShortTimeouts();
  options.SetTls(Tls::Builder()
                     .SetVerifyMode(cpp_network::comm::VerifyMode::
                                        kSkipVerification)
