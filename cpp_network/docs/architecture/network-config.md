...
**实现位置**: `src/public/include/comm/options.h`、`src/http/options.cc`
...
`cpp_network::comm::Options` 是 `Client` 的统一配置实体，内嵌 `Tls`。所有字段在 `Create()` 时经 `Validate()` 校验，映射见 http-config-mapping.md。
