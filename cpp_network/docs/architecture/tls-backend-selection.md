...
└── comm/tls.h           # Public types: cpp_network::comm::Tls / VerifyMode
```
...
| Linux x86_64 | OpenSSL（源码构建） | analysis-only · executor pending | `bazel build --config=linux_x86_64 --nobuild //src/public:cpp_network ...` rc=0；`//third_party/openssl/host:openssl/:curl` 已补 glibc 系统链接库（pthread/dl/m） |
