...
**实现位置**: `src/public/include/comm/error.h`、`src/public/include/comm/result.h`、`src/http/error.cc`、`src/http/engine.cc`（MapCurlError）

## Overview

所有公共 API 失败以 `Result<T>` 携带 `Error` 返回，不抛异常。错误码枚举 `cpp_network::comm::ErrorCode` 稳定且 append-only。
